...
export default function ServiceDetailClient({ slug }: { slug: string }) {
  ...
  const parseCustomTabs = (): CustomTab[] => {
    ...
    while (tabIndex <= 20) {
      const tabName = serviceData[`Tab ${tabIndex} Name`]
      ...
      while (itemIndex <= 50) {
        const itemText = serviceData[`Tab ${tabIndex} Item ${itemIndex}`]
        if (!itemText || itemText === "-") break

        const itemIcon = serviceData[`Tab ${tabIndex} Item ${itemIndex} Icon`]
  ...
  const parseCustomButtons = (): CustomButton[] => {
    ...
    while (btnIndex <= 10) {
      const btnText = serviceData[`Button ${btnIndex} Text`]
  ...
  const parseFAQItems = (): Array<{ question: string; answer: string }> => {
    if (!serviceData) return []
    const items: Array<{ question: string; answer: string }> = []
    let index = 1

    while (index <= 100) {
      const question = serviceData[`FAQ ${index} Question`]
      const answer = serviceData[`FAQ ${index} Answer`]
        ...
        <div className="flex-1 flex items-center justify-center bg-gray-50">
          <PageTransition>
            <div className="text-center p-8">
              ...
              <p className="text-gray-600 mb-6">Layanan "{slug}" tidak ditemukan.</p>
  ...
  const title = serviceData["Title"] || slug.replace(/-/g, " ")
      ...
      <section style={{ backgroundColor: bgColor }} className="pt-28 pb-4">
        <PageTransition>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <nav className="flex items-center gap-2 text-sm" style={{ color: `${titleColor}80` }} aria-label="Breadcrumb">
                        ...
                        return (
                          ...
                          >
                            ...
                            {hasPoints && isExpanded && (
                              <div className="px-3 md:px-4 pb-3 md:pb-4 animate-in slide-in-from-top-2 duration-300">
                                <ul className="space-y-2">
                                  {item.points.map((point, pointIndex) => (
                                    <li key={pointIndex} className="flex gap-2 text-sm md:text-base" style={{ color: textColor }}>
                                      <span className="flex-shrink-0" style={{ color: buttonColor }}>•</span>
                                      <span>{point}</span>
                                    </li>
                                  ))}
                                </ul>
      ...
      {(() => {
        ...
        if (customFAQ && customFAQ.items && customFAQ.items.length > 0) {
          ...
          finalFAQ = {
            ...
            description: "Temukan jawaban untuk pertanyaan yang sering diajukan tentang layanan kami",
            items: spreadsheetFAQItems,
          }
        } else {
          // Gunakan default FAQ dari page-content
          finalFAQ = getFAQForSlug("services", slug)
        }
