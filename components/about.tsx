...
function StatCard({ value, label, active, delay }: { value: string; label: string; active: boolean; delay: number }) {
  ...
  return (
    <div className={`bg-white/[0.05] rounded-xl p-3 md:p-5 border border-white/8 hover:border-powder/35 transition-all duration-500 hover:-translate-y-1 ${active ? "animate-fade-in-up" : "opacity-0"}`} style={{ animationDelay: `${delay}ms` }}>
      <div className="text-[22px] md:text-3xl font-bold text-powder mb-0.5">{value}</div>
      <div className="text-steel text-[11px] md:text-sm">{label}</div>
    </div>
  )
}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-16 items-center">
          ...
          <div ref={left.ref} className={`relative transition-all duration-700 ease-out ${left.inView ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10"}`}>
            <div className="bg-carbon rounded-2xl p-5 md:p-10">
              ...
              <div className="mt-4 rounded-xl bg-white/[0.04] border border-white/8 p-3 relative overflow-hidden group">
                ...
                <svg className="w-full h-20 relative z-10 opacity-80" viewBox="0 0 300 100">
                  ...
                  <circle cx="75" cy="45" r="5" fill="#a9b4c2" />
                  <circle cx="190" cy="55" r="4" fill="#eef1ef" opacity="0.5" />
                </svg>
                <div className="text-[11px] text-steel/70 text-center mt-1 relative z-10">Kalimantan Barat, Indonesia</div>
              </div>
            </div>
          </div>
          ...
          <div ref={right.ref} className={`space-y-3.5 md:space-y-6 transition-all duration-700 ease-out ${right.inView ? "opacity-100 translate-x-0" : "opacity-0 translate-x-10"}`} style={{ transitionDelay: "150ms" }}>
            <div>
              ...
              <h2 className="text-[20px] md:text-4xl font-bold text-carbon leading-tight">{data.title}</h2>
            </div>
            <p className="text-slate-brand text-[13px] md:text-lg leading-relaxed">{data.description}</p>
            <div className="flex flex-wrap gap-2 pt-1">
              <Link href={data.buttonHref} className="px-5 py-2.5 rounded-xl font-semibold bg-steel text-carbon hover:bg-powder transition-all duration-200 hover:scale-105 active:scale-95 text-[13px]">
                {data.buttonText}
              </Link>
              <Link href="/services" className="px-5 py-2.5 rounded-xl font-semibold bg-carbon text-platinum hover:bg-carbon/80 transition-all duration-200 hover:scale-105 active:scale-95 text-[13px]">
                Layanan Kami
              </Link>
            </div>
          </div>
