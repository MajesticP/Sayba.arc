...
function SocialIcon({ icon, className }: { icon: string; className: string }) {
  switch (icon) {
    ...
    default: return <span className="text-xs font-bold">{icon[0]?.toUpperCase()}</span>
  }
}

export default function Footer({ footerLinks, socialLinks }: FooterProps) {
  const { siteConfig, arcgisDepartment, itDepartment, oceanicDepartment } = useContent()
  const depts = [arcgisDepartment, itDepartment, oceanicDepartment]
  const isEn = footerLinks[0]?.label === "Home"
  const navLabel = isEn ? "Navigation" : "Navigasi"
  const contactLabel = isEn ? "Contact" : "Kontak"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5 md:gap-10 mb-6 md:mb-12">
          ...
          <div className="col-span-2 md:col-span-2">
            ...
            <p className="text-sm leading-relaxed mb-2 max-w-sm">{siteConfig.tagline}</p>
            <p className="text-xs text-white/25">{siteConfig.address}</p>

            {/* Dynamic dept pills from active language content */}
              ...
              {depts.map((dept, i) => (
                <span
                  ...
                  className={`text-[10px] md:text-xs px-2 md:px-3 py-0.5 md:py-1 rounded-full border ${
                    i === 0
                      ? "border-[#ff914d]/30 text-[#ff914d]/70"
          ...
          {/* Nav */}
          <div>
            <h4 className="text-white font-semibold text-xs md:text-sm mb-2 md:mb-4">{navLabel}</h4>
          ...
          {/* Contact */}
          <div>
            <h4 className="text-white font-semibold text-xs md:text-sm mb-2 md:mb-4">{contactLabel}</h4>
            ...
            <div className="flex gap-2.5">
              {socialLinks.map((social) => (
                <a
                  ...
                  title={social.name}
                  className="w-8 h-8 md:w-9 md:h-9 rounded-lg bg-white/5 border border-white/[0.08] flex items-center justify-center hover:bg-[#ff914d]/20 hover:border-[#ff914d]/40 hover:text-[#ff914d] transition-all duration-200"
                >
