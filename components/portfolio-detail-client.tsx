...
export default function PortfolioDetailClient({ slug }: { slug: string }) {
        ...
        <div className="flex-1 flex items-center justify-center bg-gray-50">
          <PageTransition>
            <div className="text-center p-5">
              <h1 className="text-[20px] font-bold text-gray-800 mb-2">Portfolio Tidak Ditemukan</h1>
  ...
  return (
    <main className="min-h-screen flex flex-col">
      ...
      {/* Hero Section */}
      <section style={{ backgroundColor: bgColor }} className="py-5 md:py-16 flex-1 relative overflow-hidden">
          ...
          <PageTransition delay={100}>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-12 items-start">
              ...
              <div className="order-last lg:order-first animate-in fade-in slide-in-from-bottom-8 lg:slide-in-from-left-8 duration-700">
                ...
                <h1 className="text-[20px] sm:text-2xl md:text-5xl font-bold mb-1.5 md:mb-4 capitalize" style={{ color: titleColor }}>
                  {title}
                </h1>

                {subtitle && subtitle !== "-" && (
                  <p className="text-[13px] md:text-xl mb-2.5 md:mb-6" style={{ color: buttonColor }}>
                    {subtitle}
                  </p>
                )}

                <p className="text-[12px] md:text-lg leading-relaxed mb-4 md:mb-8" style={{ color: textColor }}>
                  ...
                  <div className="flex flex-col sm:flex-row flex-wrap gap-3 md:gap-4">
                    {customButtons.map((btn, index) => (
                      <Link
                        ...
                        className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-full font-medium transition-all duration-300 hover:scale-105 hover:shadow-lg group text-[13px]"
                        style={{
          ...
          {customTabs.length > 0 && (
            <PageTransition delay={200}>
              <div className="mt-5 md:mt-16">
                ...
                <div className="flex overflow-x-auto gap-1 md:gap-2 border-b mb-4 md:mb-8 pb-0 scrollbar-hide" style={{ borderColor: `${titleColor}20` }}>
                  {customTabs.map((tab) => (
                    <button
                      key={tab.id}
                      onClick={() => setActiveTab(tab.id)}
                      className="px-3 md:px-6 py-2 md:py-3 font-medium transition-all duration-300 relative whitespace-nowrap text-[12px] md:text-base hover:bg-gray-50 rounded-t-lg"
                      style={{
                          ...
                          >
                            ...
                            >
                              <div className="flex items-center gap-3 md:gap-4">
                                ...
                                <span className="font-medium text-[12px] md:text-lg transition-all duration-300 group-hover:translate-x-1" style={{ color: titleColor }}>
                                  {item.title}
                                </span>
                              </div>
                              {hasPoints && (
                                  ...
                                  {item.points.map((point, pointIndex) => (
                                    <div key={pointIndex} className="flex items-start gap-2 md:gap-3">
                                      <div 
                                        ...
                                        style={{ backgroundColor: buttonColor }}
                                      />
                                      <span className="text-[11px] md:text-base" style={{ color: textColor }}>
                                        {point}
                                      </span>
                                    </div>
                                  ))}
