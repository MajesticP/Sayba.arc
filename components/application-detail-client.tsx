...
export default function ApplicationDetailClient({ slug }: { slug: string }) {
  ...
  const parseCustomTabs = (): CustomTab[] => {
    ...
    while (tabIndex <= 20) {
      const tabName = applicationData[`Tab ${tabIndex} Name`]
      ...
      while (itemIndex <= 50) {
        const itemText = applicationData[`Tab ${tabIndex} Item ${itemIndex}`]
        if (!itemText || itemText === "-") break

        const itemIcon = applicationData[`Tab ${tabIndex} Item ${itemIndex} Icon`]
  ...
  const parseCustomButtons = (): CustomButton[] => {
    ...
    while (btnIndex <= 10) {
      const btnText = applicationData[`Button ${btnIndex} Text`]
  ...
  const parseFAQItems = (): FAQ[] => {
    ...
    while (index <= 50) {
      const question = applicationData[`FAQ ${index} Question`]
      const answer = applicationData[`FAQ ${index} Answer`]
        ...
        <div className="flex-1 flex items-center justify-center bg-gray-50">
          <PageTransition>
            <div className="text-center p-8">
              ...
              <p className="text-gray-600 mb-6">Aplikasi "{slug}" tidak ditemukan dalam sistem.</p>
              <Link href="/applications" className="text-orange-500 hover:underline">
                Kembali ke Aplikasi
              </Link>
            </div>
          </PageTransition>
  ...
  const title = applicationData["Title"] || slug.replace(/-/g, " ")
  const subtitle = applicationData["Subtitle"] || ""
  const description = applicationData["Description"] || ""
  const icon = applicationData["Icon"] || "star"
  const imageUrl = applicationData["Image URL"] || ""
          ...
          {(() => {
            ...
            if (customFAQ && customFAQ.items && customFAQ.items.length > 0) {
              ...
              finalFAQ = {
                ...
                description: "Temukan jawaban untuk pertanyaan yang sering diajukan tentang aplikasi kami",
                items: spreadsheetFAQItems,
              }
            } else {
              // Gunakan default FAQ dari page-content
              finalFAQ = getFAQForSlug("applications", slug)
            }
