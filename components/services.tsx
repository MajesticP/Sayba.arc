...
export default function Services({ allLayanan, depts }: { allLayanan: Layanan[]; depts: LayananDept[] }) {
  ...
  return (
    <section className="py-10 md:py-28 bg-platinum" id="services">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={header.ref} className={`mb-7 md:mb-16 transition-all duration-700 ease-out ${header.inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-steel/15 mb-3">
            <span className="w-1.5 h-1.5 rounded-full bg-steel animate-pulse" />
            <span className="text-[10px] font-bold uppercase tracking-widest text-slate-brand">Layanan Unggulan</span>
          </div>
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
            <div>
              <h2 className="text-[22px] leading-tight md:text-5xl font-black text-carbon">
                ...
                <span className="text-slate-brand"> untuk Anda</span>
              </h2>
            ...
            <Link href="/services" className="inline-flex items-center gap-1.5 text-[13px] font-semibold text-slate-brand hover:text-carbon transition-colors group self-start md:self-auto">
              Lihat semua layanan
        ...
        <div ref={cards.ref} className="flex gap-4 overflow-x-auto snap-x snap-mandatory pb-2 -mb-2 sm:grid sm:grid-cols-2 xl:grid-cols-3 sm:gap-4 md:gap-5 sm:overflow-visible sm:pb-0 sm:mb-0 scrollbar-hide">
          {allLayanan.slice(0, 3).map((service, i) => {
            ...
            const color = deptCfg?.color ?? "#5e6572"
            ...
            return (
              <Link key={service.id} href={service.slug ? `/services/${service.slug}` : "#"}
                className={`group relative transition-all duration-400 hover:-translate-y-1 hover:shadow-2xl overflow-hidden rounded-2xl border border-carbon/10 flex-shrink-0 w-[86vw] sm:w-auto snap-center flex flex-col ${cards.inView ? "animate-fade-in-up" : "opacity-0"}`}
                style={{
                  "--card-color": color,
                ...
                <div className="relative w-full bg-carbon/5 overflow-hidden leading-[0]" style={{ height: "160px" }}>
                  {imgSrc ? (
                  ...
                  <div className="absolute top-2 right-2 w-5 h-5 rounded-full bg-white/80 backdrop-blur-sm border border-carbon/8 flex items-center justify-center shadow-sm">
                    <span className="text-[8px] font-black text-carbon/40">0{i + 1}</span>
                  </div>
                </div>

                {/* Content — hard-capped at remaining space */}
                <div
                  className="flex flex-col p-4 bg-white group-hover:bg-carbon/[0.01] transition-colors duration-300 flex-1"
                >
                  ...
                  <h3 className="text-[14px] font-black text-carbon mb-1.5 leading-snug group-hover:text-carbon/80 transition-colors flex-shrink-0 line-clamp-2">
                    {service.title}
                  </h3>
                  <p className="text-carbon/45 text-[12px] leading-relaxed line-clamp-3">
                    {service.description ?? ""}
                  </p>
        ...
        <div className="flex items-center justify-center gap-1.5 mt-3 sm:hidden">
          {allLayanan.slice(0, 3).map((_, i) => (
            <span key={i} className="w-1.5 h-1.5 rounded-full bg-carbon/15" />
          ))}
          <span className="text-[10px] text-carbon/25 ml-1.5">Geser untuk lihat lainnya</span>
        </div>

        {/* Bottom strip */}
        <div className={`mt-6 md:mt-12 flex flex-col sm:flex-row items-center justify-between gap-3 py-4 border-t border-carbon/8 transition-all duration-700 ${cards.inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`} style={{ transitionDelay: "500ms" }}>
          <p className="text-carbon/35 text-[13px] text-center sm:text-left">Tidak menemukan yang Anda cari?</p>
          <Link href="/services" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-carbon text-platinum text-[13px] font-semibold hover:bg-steel hover:text-carbon transition-colors duration-200 hover:scale-105 active:scale-95">
