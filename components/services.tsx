...
export default function Services({ allLayanan, depts }: { allLayanan: Layanan[]; depts: LayananDept[] }) {
  ...
  return (
    <section className="pt-4 pb-10 md:pt-6 md:pb-20 bg-black" id="services">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={header.ref} className={`mb-7 md:mb-16 transition-all duration-700 ease-out ${header.inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#ea580c]/10 mb-3">
            <span className="w-1.5 h-1.5 rounded-full bg-[#ea580c] animate-pulse" />
            <span className="text-[10px] font-bold uppercase tracking-widest text-[#ea580c]">Layanan Unggulan</span>
          </div>
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
            <div>
                          <h2 className="text-[22px] leading-tight md:text-5xl font-black text-white">
                            Solusi Terbaik<br className="hidden md:block" />
                            <span className="text-[#ea580c]"> untuk Anda</span>
                          </h2>
                          <p className="text-white/50 text-[13px] md:text-base mt-2 max-w-md">
                            Layanan paling diminati klien kami — dipilih langsung dari portofolio aktif.
                          </p>
                        </div>
                        <Link href="/services" className="inline-flex items-center gap-1.5 text-[13px] font-semibold text-white/50 hover:text-white transition-colors group self-start md:self-auto">
              Lihat semua layanan
        ...
        <div ref={cards.ref} className="flex gap-4 overflow-x-auto snap-x snap-mandatory pb-2 -mb-2 sm:flex-col sm:gap-4 md:gap-5 sm:overflow-visible sm:pb-0 sm:mb-0 scrollbar-hide">
          {allLayanan.slice(0, 4).map((service, i) => {
                      ...
                      const color = deptCfg?.color ?? "#ea580c"
                      ...
                      return (
                        <Link key={service.id} href={service.slug ? `/services/${service.slug}` : "#"}
                          className={`group relative transition-all duration-400 hover:-translate-y-1 hover:shadow-2xl overflow-hidden rounded-2xl border border-white/10 flex-shrink-0 w-[86vw] sm:w-full snap-center flex flex-col sm:flex-row bg-white/5 ${cards.inView ? "animate-card-reveal" : "opacity-0"}`}
                          style={{
                            "--card-color": color,
                          ...
                          <div className="relative w-full sm:w-[35%] bg-black/20 overflow-hidden shrink-0" style={{ height: "180px", minHeight: "100%" }}>
                            {imgSrc ? (
                            ...
                            <div className="absolute top-2 right-2 w-5 h-5 rounded-full bg-white/10 backdrop-blur-sm border border-white/10 flex items-center justify-center shadow-sm">
                              <span className="text-[8px] font-black text-white/40">0{i + 1}</span>
                            </div>
                          </div>

                          {/* Content — hard-capped at remaining space */}
                          <div
                            className="flex flex-col p-4 sm:p-6 lg:p-8 bg-white/[0.02] group-hover:bg-white/[0.05] transition-colors duration-300 flex-1"
                          >
                            ...
                            <h3 className="text-[15px] sm:text-[18px] lg:text-[20px] font-black text-white mb-2 leading-snug group-hover:text-[#ea580c] transition-colors flex-shrink-0 line-clamp-2">
                              {service.title}
                            </h3>
                            <p className="text-white/45 text-[12px] sm:text-[13px] leading-relaxed line-clamp-3 sm:line-clamp-4">
                              {service.description ?? ""}
                            </p>
        ...
        {/* Swipe hint — mobile only */}
                <div className="flex items-center justify-center gap-1.5 mt-3 sm:hidden">
                  ...
                  <span className="text-[10px] text-white/25 ml-1.5">Geser untuk lihat lainnya</span>
                </div>

                {/* Bottom strip */}
                <div className={`mt-6 md:mt-12 flex flex-col sm:flex-row items-center justify-between gap-3 py-4 border-t border-white/8 transition-all duration-700 ${cards.inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`} style={{ transitionDelay: "500ms" }}>
                  <p className="text-white/35 text-[13px] text-center sm:text-left">Tidak menemukan yang Anda cari?</p>
                  <Link href="/services" className="btn-shine inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-black text-[13px] font-semibold hover:bg-[#ea580c] hover:text-white transition-colors duration-200 hover:scale-105 active:scale-95">
